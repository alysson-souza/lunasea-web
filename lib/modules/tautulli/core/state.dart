...
class TautulliState extends LunaModuleState {
  ...
  void resetStatistics() {
    if (_api != null)
      _statistics = _api!.history.getHomeStats(
        ...
        statsCount: TautulliPreferences.STATISTICS_STATS_COUNT.read(),
      );
    notifyListeners();
  }
  ...
  void resetRecentlyAdded() {
    if (_api != null)
      _recentlyAdded = _api!.libraries.getRecentlyAdded(
        count: TautulliPreferences.CONTENT_LOAD_LENGTH.read(),
      );
    notifyListeners();
  }
  ...
  void resetDailyPlayCountGraph() {
    if (_api != null)
      _dailyPlayCountGraph = _api!.history.getPlaysByDate(
        timeRange: TautulliPreferences.GRAPHS_LINECHART_DAYS.read(),
        yAxis: _graphYAxis,
      );
    notifyListeners();
  }
  ...
  void resetPlaysByMonthGraph() {
    if (_api != null)
      _playsByMonthGraph = _api!.history.getPlaysPerMonth(
        timeRange: TautulliPreferences.GRAPHS_MONTHS.read(),
        yAxis: _graphYAxis,
      );
    notifyListeners();
  }
  ...
  void fetchSearch() {
    _search = _api!.libraries.search(
      query: _searchQuery,
      limit: TautulliPreferences.CONTENT_LOAD_LENGTH.read(),
    );
    notifyListeners();
  }
