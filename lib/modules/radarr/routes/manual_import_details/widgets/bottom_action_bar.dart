...
class RadarrManualImportDetailsBottomActionBar extends StatelessWidget {
    ...
    return LunaBottomActionBar(
      actions: [
        Consumer<SettingsStore>(
          builder: (context, settings, _) => LunaActionBarCard(
            title: 'radarr.ImportMode'.tr(),
            subtitle: RadarrImportMode.COPY
                .from(settings.radarrManualImportDefaultMode)!
                .lunaReadable,
            //checkboxState: true,
  ...
  Future<void> _importModeOnTap(BuildContext context) async {
    ...
    if (result.item1) {
      await context
          .read<SettingsStore>()
          .setRadarrManualImportDefaultMode(result.item2!.value);
    }
  }
        ...
        await RadarrAPIHelper()
            .triggerManualImport(
              ...
              importMode: RadarrImportMode.COPY
                  .from((RadarrPreferences.MANUAL_IMPORT_DEFAULT_MODE.read()))!,
            )
            .then((result) => result
