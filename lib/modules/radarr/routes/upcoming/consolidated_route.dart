...
/// Upcoming tab for the consolidated Radarr view.
///
/// Merges the upcoming lists from all enabled Radarr instances.
class RadarrConsolidatedUpcomingRoute extends StatefulWidget {
  const RadarrConsolidatedUpcomingRoute({super.key});

  @override
  State<RadarrConsolidatedUpcomingRoute> createState() => _State();
}

class _State extends State<RadarrConsolidatedUpcomingRoute>
  ...
  Future<void> _refresh() async {
    ...
    if (consolidated.upcoming != null &&
        consolidated.qualityProfilesByInstance != null) {
      await Future.wait([
        consolidated.upcoming!,
  ...
  Widget get _body => LunaRefreshIndicator(
    ...
    child:
        Selector<
          ...
          >
        >(
          selector: (_, s) => Tuple2(s.upcoming, s.qualityProfilesByInstance),
              ...
              builder: (context, AsyncSnapshot<List<Object>> snapshot) {
                if (snapshot.hasError) {
                  if (snapshot.connectionState != ConnectionState.waiting) {
                    LunaLogger().error(
                      'Unable to fetch consolidated Radarr upcoming',
                      snapshot.error,
                      snapshot.stackTrace,
                    );
                  }
