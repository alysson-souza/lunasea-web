...
class RadarrDialogs {
      ...
      content: [
        Consumer<SettingsStore>(
          builder: (context, settings, _) => LunaDialog.checkbox(
            ...
            value: settings.radarrQueueRemoveFromClient,
            onChanged: (selected) => context
                .read<SettingsStore>()
                .setRadarrQueueRemoveFromClient(selected!),
          ),
        ),
        Consumer<SettingsStore>(
          builder: (context, settings, _) => LunaDialog.checkbox(
            title: 'Blacklist Release',
            value: settings.radarrQueueBlacklist,
            onChanged: (selected) => context
                .read<SettingsStore>()
                .setRadarrQueueBlacklist(selected!),
          ),
        ),
      ],
      contentPadding: LunaDialog.listDialogContentPadding(),
      ...
      content: [
        Consumer<SettingsStore>(
          builder: (context, settings, _) => LunaDialog.checkbox(
            ...
            value: settings.radarrRemoveMovieImportList,
            onChanged: (value) => context
                .read<SettingsStore>()
                .setRadarrRemoveMovieImportList(value!),
          ),
        ),
        Consumer<SettingsStore>(
          builder: (context, settings, _) => LunaDialog.checkbox(
            title: 'Delete Files',
            value: settings.radarrRemoveMovieDeleteFiles,
            onChanged: (value) => context
                .read<SettingsStore>()
                .setRadarrRemoveMovieDeleteFiles(value!),
          ),
        ),
      ],
      contentPadding: LunaDialog.listDialogContentPadding(),
  ...
  Future<Tuple2<bool, int>> setQueuePageSize(BuildContext context) async {
    ...
    TextEditingController _textController = TextEditingController(
      text: RadarrPreferences.QUEUE_PAGE_SIZE.read().toString(),
    );
  ...
  Future<void> addMovieOptions(BuildContext context) async {
    await LunaDialog.dialog(
      ...
      content: [
        Consumer<RadarrAddMovieDetailsState>(
          builder: (context, state, _) => LunaDialog.checkbox(
            title: 'radarr.StartSearchForMissingMovie'.tr(),
            value: state.searchForMovie,
            onChanged: (value) =>
                context.read<RadarrAddMovieDetailsState>().searchForMovie =
                    value!,
          ),
        ),
      ],
