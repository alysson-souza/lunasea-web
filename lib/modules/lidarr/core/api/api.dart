...
class LidarrAPI {
  ...
  factory LidarrAPI.from(LunaProfile profile) {
    ...
    return LidarrAPI.fromInstance(instances.first);
  }

  factory LidarrAPI.fromInstance(LunaServiceInstance instance) {
  ...
  Future<bool> refreshArtist(int artistID) async {
    try {
      await _dio.post(
        'command',
        data: json.encode({'name': 'RefreshArtist', 'artistId': artistID}),
      );
      return true;
    } on DioException catch (error, stack) {
  ...
  Future<bool> editArtist(
    ...
    try {
      ...
      response = await _dio.put('artist', data: json.encode(artist));
      return true;
    } on DioException catch (error, stack) {
  ...
  Future<LidarrAlbumData> getAlbum(int albumID) async {
    try {
      Response response = await _dio.get(
        'album',
        queryParameters: {'albumIds': albumID},
      );
  ...
  Future<bool> searchAlbums(List<int> albums) async {
    try {
      await _dio.post(
        'command',
        data: json.encode({'name': 'AlbumSearch', 'albumIds': albums}),
      );
      return true;
    } on DioException catch (error, stack) {
      logError(
        'Failed to search for albums (${albums.toString()})',
        error,
        stack,
      );
      return Future.error(error);
  ...
  Future<bool> updateLibrary() async {
    try {
      await _dio.post('command', data: json.encode({'name': 'RefreshArtist'}));
      return true;
    } on DioException catch (error, stack) {
  ...
  Future<bool> triggerRssSync() async {
    try {
      await _dio.post('command', data: json.encode({'name': 'RssSync'}));
      return true;
    } on DioException catch (error, stack) {
  ...
  Future<bool> triggerBackup() async {
    try {
      await _dio.post('command', data: json.encode({'name': 'Backup'}));
      return true;
    } on DioException catch (error, stack) {
  ...
  Future<bool> toggleArtistMonitored(int artistID, bool status) async {
    try {
      ...
      response = await _dio.put('artist', data: json.encode(body));
      return true;
    } on DioException catch (error, stack) {
      logError(
        'Failed to toggle artist monitored status ($artistID)',
        error,
        stack,
      );
      return Future.error(error);
  ...
  Future<bool> toggleAlbumMonitored(int albumID, bool status) async {
    try {
      ...
      body['monitored'] = status;
      response = await _dio.put('album', data: json.encode(body));
      return true;
    } on DioException catch (error, stack) {
      logError(
        'Failed to toggle album monitored status ($albumID)',
        error,
        stack,
      );
      return Future.error(error);
  ...
  Future<bool> downloadRelease(String guid, int indexerId) async {
    try {
      await _dio.post(
        'release',
        data: json.encode({'guid': guid, 'indexerId': indexerId}),
      );
      return true;
    } on DioException catch (error, stack) {
