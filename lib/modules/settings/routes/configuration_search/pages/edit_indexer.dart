...
class _State extends State<ConfigurationSearchEditIndexerRoute>
  ...
  Widget build(BuildContext context) {
    final store = context.watch<IndexersStore>();
    if (widget.id < 0 || store.read(widget.id) == null) {
        ...
        LunaButton.text(
          ...
          onTap: () async {
            ...
            if (result) {
              ...
              await context.read<IndexersStore>().delete(widget.id);
  ...
  Widget _body() {
    return Consumer<IndexersStore>(
      builder: (context, indexers, _) {
        _indexer = indexers.read(widget.id);
    ...
    return LunaBlock(
      ...
      onTap: () async {
        ...
        if (values.item1) {
          ...
          await context.read<IndexersStore>().update(widget.id, _indexer!);
        }
      },
    );
  }
