...
extension SettingsBannersExtension on SettingsBanners {
  ...
  bool shouldShow(BuildContext context) =>
      context.watch<DismissedBannersStore>().shouldShow(key);

  /// Mark the banner as seen, so it will not appear in the UI anymore
  Future<void> markSeen(BuildContext context) =>
      context.read<DismissedBannersStore>().dismiss(key);
  ...
  Widget banner({
    ...
    return Consumer<DismissedBannersStore>(
      builder: (context, banners, _) {
        if (banners.shouldShow(key))
          return LunaBanner(
            dismissCallback: () => markSeen(context),
            headerText: header,
            bodyText: body,
            icon: icon,
            iconColor: iconColor,
