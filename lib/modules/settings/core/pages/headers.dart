...
class _State extends State<SettingsHeaderRoute> with LunaScrollControllerMixin {
  ...
  Widget _body() {
    return Consumer<ProfilesStore>(
      builder: (context, store, _) => LunaListView(
        controller: scrollController,
        children: [
          if ((_headers(store)).isEmpty) _noHeadersFound(),
          ..._headerList(store),
        ],
      ),
    );
  ...
  List<LunaBlock> _headerList(ProfilesStore store) {
    final headers = _headers(store);
    List<String> _sortedKeys = headers.keys.toList()..sort();
    return _sortedKeys
        .map<LunaBlock>((key) => _headerBlock(store, key, headers[key]))
        .toList();
  }

  LunaBlock _headerBlock(ProfilesStore store, String key, String? value) {
    return LunaBlock(
      title: key,
  ...
  Map<String, String> _headers(ProfilesStore store) {
    final profile = store.active;
