...
class SonarrSeriesAddDetailsState extends ChangeNotifier {
  ...
  void initializeMonitored() {
    _monitored = SonarrPreferences.ADD_SERIES_DEFAULT_MONITORED.read();
  }
  ...
  void initializeRootFolder(List<SonarrRootFolder> rootFolders) {
    _rootFolder = rootFolders.firstWhere(
      (element) =>
          element.id == SonarrPreferences.ADD_SERIES_DEFAULT_ROOT_FOLDER.read(),
  ...
  set tags(List<SonarrTag> tags) {
    _tags = tags;
    SonarrPreferences.ADD_SERIES_DEFAULT_TAGS
  ...
  void initializeTags(List<SonarrTag> tags) {
    _tags = tags
        .where((tag) =>
            (SonarrPreferences.ADD_SERIES_DEFAULT_TAGS.read()).contains(tag.id))
        .toList();
  }
