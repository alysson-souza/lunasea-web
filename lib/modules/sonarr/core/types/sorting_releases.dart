...
class _Sorter {
  ...
  List<SonarrRelease> _alphabetical(
    ...
    ascending
        ? releases.sort((a, b) => (a.title ?? '')
            .toLowerCase()
            .compareTo((b.title ?? '').toLowerCase()))
        : releases.sort((a, b) => (b.title ?? '')
            .toLowerCase()
  ...
  List<SonarrRelease> _weight(List<SonarrRelease> releases, bool ascending) {
    ascending
        ? releases.sort(
            (a, b) => (a.releaseWeight ?? -1).compareTo(b.releaseWeight ?? -1))
        : releases.sort(
            (a, b) => (b.releaseWeight ?? -1).compareTo(a.releaseWeight ?? -1));
    return releases;
  }
  ...
  List<SonarrRelease> _wordScore(List<SonarrRelease> releases, bool ascending) {
    ascending
        ? releases.sort((a, b) =>
            (a.preferredWordScore ?? -1).compareTo(b.preferredWordScore ?? -1))
        : releases.sort((a, b) =>
            (b.preferredWordScore ?? -1).compareTo(a.preferredWordScore ?? -1));
    return releases;
  }
}
